...
public class DefaultCoverageGenerator implements CoverageGenerator {
    ...
    private void gatherCoverageData(final Collection<ClassInfo> tests, final CoverageData coverage) throws IOException, InterruptedException, ExecutionException {
        ...
        final CoverageProcess process = new CoverageProcess(ProcessArgs.withClassPath(this.code.getClassPath()).andBaseDir(this.workingDir).andLaunchOptions(this.launchOptions).andStderr(logInfo()).andStdout(captureStandardOutIfVerbose()),
        ...
        if (exitCode == ExitCode.JUNIT_ISSUE) {
            ...
            throw new PitError("Coverage generation minion exited abnormally. Please check that your classpath contains JUnit 4.6 or above.");
