 ...
 * 
 * It will probably work well on your legacy code base but some of the code is pretty
 * hairy, so support for more modern test frameworks will likely be added via
 * different plugins.
 * 
 * Much of the complexity comes from splitting the tests down into smaller
 * units.
 *
 * Note. No service locator property file is provided for this plugin, it
 * is hard coded to be used when no other is provided on the classpath.
 */
