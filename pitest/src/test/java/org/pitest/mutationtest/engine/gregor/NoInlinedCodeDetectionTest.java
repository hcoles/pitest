...
public class NoInlinedCodeDetectionTest {
  ...
  public void shouldReturnSuppliedMutationsUnchanged() {
    NoInlinedCodeDetection testee = new NoInlinedCodeDetection();
    Collection<MutationDetails> mutations = Arrays.asList(makeMutation());
    assertSame(mutations,testee.process(mutations));
  }
  
  private MutationDetails makeMutation() {
    return new MutationDetails(new MutationIdentifier("foo",1, "foo"),null,null,null,0, 0);
  }
