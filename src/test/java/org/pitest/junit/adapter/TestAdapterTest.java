...
public class TestAdapterTest {
  ...
  public void testCallsNotifyStart() {
    ...
    verify(this.rc).notifyStart(testee.description());
  }
  ...
  public void testCallsNotifyEndOnSuccess() {
    final TestAdapter testee = new TestAdapter(new HideFromJUnit.JMockTest(
        "testPasses"));
    testee.execute(IsolationUtils.getContextClassLoader(), this.rc);
    verify(this.rc).notifyEnd(testee.description());
  }

  @Test
  public void testCallsNotifyEndWithThrowableOnFailure() {
    final TestAdapter testee = new TestAdapter(new HideFromJUnit.JMockTest(
        "testFails"));
    testee.execute(IsolationUtils.getContextClassLoader(), this.rc);
    verify(this.rc).notifyEnd(eq(testee.description()), any(Throwable.class));
  }

  @Test
  public void testCanSerializeAndDeserializeAnAdapterJMockTest() {
    final TestAdapter testee = new TestAdapter(new HideFromJUnit.JMockTest(
        "testPasses"));
    ...
    verify(this.rc).notifyEnd(actual.description());
  }
